...
personalUse: "Every box I own answers on a different port with a different key, and two of them are only reachable through a jump host. Before I kept a real config, every connection was a paragraph of flags I had to remember or scroll back to find. Now it is one short alias, and the file stays readable enough that I can fix it a year later."
status: "active"
