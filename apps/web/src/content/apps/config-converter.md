...
personalUse: "I move config between formats constantly - a Docker env into a YAML values file, a JSON snippet into TOML for a Rust project, a settings block into a .env for local dev. The web converters I found either paywall it, wrap it in ads, or - worse - upload whatever I paste, which is a non-starter when the config has secrets in it. I wanted one that runs entirely in the tab and never phones home."
status: "active"
