...
Every cloud LLM API hides the same arithmetic behind the same headline. The per-million-tokens rate looks flat. It isn't. Output is three to ten times input. Reasoning models bury chain-of-thought tokens inside that output bill. Every request re-pays the cost of the system prompt and any RAG context attached to it. None of that is on the pricing page. All of it lands on the invoice.
...
This post is partly synthesis and partly my own roadmap. I run local inference today on a Mac Mini and a Pi cluster, with the [GekroLLMClient](/blog/api-sovereignty) routing between cloud and local based on workload class. What I haven't done yet — and what the next quarter of lab work is built around — is running an end-to-end QLoRA against a curated dataset on my own hardware, with the eval loop and MLOps discipline that turn a one-off fine-tune into a reproducible pipeline. The post-mortem on actually doing it is the next Deep Dive in this thread. The one after that ties the economics back to the Texas grid: ERCOT pays industrial consumers to curtail load during stress events, and a grid-aware MLOps pipeline that schedules its training against demand-response windows turns *owning the means of production* into *owning the means of production while the grid pays you for behaving well*.
