...
Here is the part that makes this an engineer's review and not a coupon. I liked Wispr enough that I wanted to own the workflow instead of renting it, so I forked [whisper-key-local](https://github.com/PinW/whisper-key-local) by Pin Wang and then kept going. [Whisper Local](https://github.com/drajb/whisper-local) is now more than 40 commits ahead of the upstream I started from, and still moving: a profiles system, per-app rules, an inline voice-command DSL, a transforms-and-dictionary layer, a floating capture overlay, an opt-in `whisper.cpp` backend, and a stats dashboard, all bent toward one goal - the Wispr feel, running locally. I have been on it daily for weeks.
