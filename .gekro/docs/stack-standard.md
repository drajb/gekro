...
| **No banners, no popups, no "use this code for 20% off" CTAs.** Referral links are single, unobtrusive, footer-positioned. | Visible monetization breaks the "verified by an engineer" persona instantly. |
...
- `<StackVerdict>` — pull-quote-style verdict block
...
- **One muted line at the bottom of each entry**, via `<StackReferralFooter>`. `text-text-muted text-xs italic`. No banner, no bold, no "as an affiliate..." legalese.
...
*Last updated: 2026-05-15 (initial spec)*
