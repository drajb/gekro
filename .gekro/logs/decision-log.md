...
| 2026-03-24 04:45 | Env-Var-Only Secrets | All API keys, analytics IDs, and service keys must come from env vars (`import.meta.env` / `process.env`). No hardcoded fallbacks in source. | Added `.env.example` as the single source of truth for required vars. | 0 |
