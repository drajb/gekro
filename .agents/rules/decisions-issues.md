...
**Maintenance**: Keep these logs updated in REAL-TIME with every prompt and implementation (file edits), but only commit/push these updates as part of a batched commit when the technical task is complete or when the USER requests it.
